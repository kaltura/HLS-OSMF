...
package org.osmf.net.httpstreaming
{
	 ...
	 * @private
	 * 
	 * HLSHTTPStreamMixer class supports mixing alternate and media data
	 * provided by different sources. This class is modified to support
	 * signed FLV tags.
	 * 
	 * @langversion 3.0
	 ...
	 */
	public class HLSHTTPStreamMixer extends EventDispatcher implements IHTTPStreamSource
	{
		/**
		 * Default constructor.
		 */
		public function HLSHTTPStreamMixer(dispatcher:IEventDispatcher)
		{
			if (dispatcher == null)
			{
		...
		protected function doSomeProcessingAndGetBytes():ByteArray
		{
			...
			switch(_state)
			{
				...
				case HTTPStreamingState.READ:
					...
					if (_alternateNeedsSynchronization && (_mediaTime != int.MIN_VALUE || _alternateTime != int.MIN_VALUE))
					{
					...
					if (_mediaNeedsMoreData || _alternateNeedsMoreData)
					{
						...
						if (!_alternateIgnored && _alternateNeedsMoreData && _alternateHandler != null && _alternateHandler.source.isReady)
						{
							...
							if(alternateBytes && alternateBytes.length > 0)
								trace("    - alternateBytes length was " + alternateBytes.length);
						...
						if (_mediaNeedsMoreData && _mediaHandler != null && _mediaHandler.source.isReady)
						{
							...
							if(mediaBytes && mediaBytes.length > 0)
								trace("    - mediaBytes length was " + mediaBytes.length);
						}
			...
			if(bytes)
				trace("RETURNING " + bytes.length + " MIXED BYTES");
			return bytes;
		}
		...
		private function shouldFilterTag(tag:FLVTag, filterTags:uint):Boolean
		{
			...
			switch (tag.tagType)
			{
				...
				case FLVTag.TAG_TYPE_VIDEO:
				case FLVTag.TAG_TYPE_ENCRYPTED_VIDEO:
					return (FILTER_VIDEO & filterTags) || (wrappedTimestamp < _mediaTime);
					break;
				
				case FLVTag.TAG_TYPE_SCRIPTDATAOBJECT:
				case FLVTag.TAG_TYPE_ENCRYPTED_SCRIPTDATAOBJECT:
					return (FILTER_DATA & filterTags) || (wrappedTimestamp < _mediaTime);
					break;
			}	
		...
		private var _mediaTime:int = int.MIN_VALUE;
		private var _alternateTime:int = int.MIN_VALUE;
		...
		CONFIG::LOGGING
		{
			private static const logger:Logger = Log.getLogger("org.osmf.net.httpstreaming.HLSHTTPStreamMixer");
